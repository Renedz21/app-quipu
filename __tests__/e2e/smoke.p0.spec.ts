...
test.describe("P0 smoke @smoke", () => {
  test(
    "dashboard carga para usuario con profile",
    { tag: "@smoke" },
    async ({ authedPage, convexClient }) => {
      ...
      await expect(
        authedPage.getByRole("heading", { name: "Dashboard (próximamente)" }),
      ).toBeVisible();
      await expect(
        authedPage.getByText("El dashboard real se implementa"),
      ).toBeVisible();
  ...
  test(
    "getMyProfile responde sin error para usuario autenticado",
    { tag: "@smoke" },
    async ({ convexClient }) => {
      await seedOnboardedUser(convexClient);
  ...
  test(
    "registrar gasto baja el saldo del sobre Gustos",
    { tag: "@smoke" },
    async ({ convexClient }) => {
      await seedOnboardedUser(convexClient);
  ...
  test(
    "resolver coachInteraction marca la interacción como resuelta",
    { tag: "@smoke" },
    async ({ convexClient }) => {
      await seedOnboardedUser(convexClient);
      await seedActiveCycle(convexClient, 100_000);
